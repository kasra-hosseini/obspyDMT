    ...
    if request == 'event-based':
        ...
        if input['event_catalog'] == 'EMSC':
            ...
            events = client_neries.getEvents(min_datetime=input['min_date'], max_datetime=input['max_date'],
                                             ...
                                             min_latitude=evlatmin, max_latitude=evlatmax, min_longitude=evlonmin,
            ...
            try:
                ...
                events_QML = client_fdsn.get_events(minlatitude=evlatmin, maxlatitude=evlatmax, minlongitude=evlonmin,
                                                    ...
                                                    limit=input['max_result'], orderby='time',
                ...
                for i in range(len(events_QML)):
                    event_time = events_QML.events[i].preferred_origin().time or events_QML.events[i].origins[0].time
        ...
        if t_cont > input['interval']:
            ...
            events.append({'author': 'NAN', 'event_id': 'continuous0', 'origin_id': -12345.0,
                           'longitude': -12345.0, 'latitude': -12345.0,
                            'datetime': m_date, 't1': m_date, 't2': M_date, 'depth': -12345.0, 'magnitude': -12345.0,
...
def IRIS_available(input, event, target_path, event_number):
    ...
    try:
        available = client_fdsn.get_stations(network=input['net'], station=input['sta'], location=input['loc'],
                                             channel=input['cha'], starttime=UTCDateTime(event['t1']),
                                             endtime=UTCDateTime(event['t2']),
                                             latitude=input['lat_cba'], longitude=input['lon_cba'],
                                             minradius=input['mr_cba'], maxradius=input['Mr_cba'],
                                             minlatitude=input['mlat_rbb'], maxlatitude=input['Mlat_rbb'],
                                             minlongitude=input['mlon_rbb'], maxlongitude=input['Mlon_rbb'],
                                             level='channel')
...
def IRIS_waveform(input, Sta_req, i, type):
    """
    Gets Waveforms, Response files and meta-data from IRIS DMC based on the requested events...
    """
    ...
    if input['iris_bulk'] == 'Y':
        try:
            ...
            for bulk_st in bulk_smgrs:
                ...
                bulk_st.write(os.path.join(add_event[i], 'BH_RAW', '%s.%s.%s.%s' % (bulk_st.stats['network'],
        ...
        # Following parameter is set to 'N' to avoid retrieving the waveforms twice
        # When using bulk requests, waveforms are retreived in bulk but not response files and not metadata
        input['waveform'] = 'N'
        t22 = datetime.now()
    ...
    if input['iris_bulk'] == 'Y':
        ...
        for line in fileinput.FileInput(os.path.join(add_event[i], 'info', 'station_event')):
            if not '%s.%s.%s.%s' % (line.split(',')[0], line.split(',')[1], line.split(',')[2], line.split(',')[3]) in \
                    sta_saved_list:
                pass
            else:
    ...
    report.writelines('---------------%s---------------\n' % input['cha'])
    rep = 'IRIS-Available stations for channel %s and for event-%s: %s\n' % (input['cha'], i, len(Sta_req))
    report.writelines(rep)
    rep = 'IRIS-%s stations for channel %s and for event-%s: %s\n' % (type, input['cha'], i, len_sta_ev)
    report.writelines(rep)
    report.writelines('----------------------------------\n')

    t_wave = datetime.now() - t_wave_1

    rep = 'Time for %sing Waveforms from IRIS: %s\n' % (type, t_wave)
    report.writelines(rep)
    report.writelines('----------------------------------\n')
    report.close()
    ...
    print "\n------------------------"
    print 'IRIS for event-%s is Done' % (i+1)
...
def IRIS_download_core(i, j, dic, type, len_events, events, add_event, Sta_req, input, client_fdsn):
    """
    ...
    try:
        ...
        if input['waveform'] == 'Y':
            dummy = 'Waveform'
            client_fdsn.get_waveforms(Sta_req[j][0], Sta_req[j][1], Sta_req[j][2], Sta_req[j][3], t_start, t_end,
                                      filename=os.path.join(add_event[i], 'BH_RAW', '%s.%s.%s.%s' % (Sta_req[j][0],
                                                                                                     Sta_req[j][1],
                                                                                                     Sta_req[j][2],
                                                                                                     Sta_req[j][3])))
        ...
        if input['time_iris'] == 'Y':
            time_iris = t22 - t11
            time_file = open(os.path.join(add_event[i], 'info', 'time_iris'), 'a')
            size = getFolderSize(os.path.join(add_event[i]))
            ti = '%s,%s,%s,%s,%s,%s,%s,+,\n' % (Sta_req[j][0], Sta_req[j][1], Sta_req[j][2], Sta_req[j][3],
                                                time_iris.seconds, time_iris.microseconds, size/(1024.**2))
        ...
        if len(Sta_req[j]) != 0:
            print '%s%s---%s.%s.%s.%s' % (info_req, dummy, Sta_req[j][0], Sta_req[j][1], Sta_req[j][2], Sta_req[j][3])
            ee = 'iris -- %s---%s-%s---%s.%s.%s.%s---%s\n' % (dummy, i, j, Sta_req[j][0], Sta_req[j][1], Sta_req[j][2],
                                                              Sta_req[j][3], e)
...
def calculate_time_phase(event, sta):
    """
    calculate arrival time of the requested phase to use in retrieving waveforms.
    """
...
def ARC_download_core(i, j, dic, type, len_events, events, add_event, Sta_req, input, client_arclink, client_neries):
    ...
    try:
        ...
        if input['paz'] == 'Y':
            dummy = 'PAZ'
            paz_arc = client_arclink.getPAZ(Sta_req[j][0], Sta_req[j][1], Sta_req[j][2], Sta_req[j][3], time=t_start)
            paz_file = open(os.path.join(add_event[i], 'Resp', 'PAZ.%s.%s.%s.%s.paz' % (Sta_req[j][0], Sta_req[j][1],
                                                                                        Sta_req[j][2], Sta_req[j][3])),
                            'w')
        ...
        if input['time_arc'] == 'Y':
            time_arc = t22 - t11
            time_file = open(os.path.join(add_event[i], 'info', 'time_arc'), 'a')
            size = getFolderSize(os.path.join(add_event[i]))
            ti = '%s,%s,%s,%s,%s,%s,%s,+,\n' % (Sta_req[j][0], Sta_req[j][1], Sta_req[j][2], Sta_req[j][3],
                                                time_arc.seconds, time_arc.microseconds, size/(1024.**2))
            time_file.writelines(ti)
            time_file.close()
    except Exception as e:
        t22 = datetime.now()
...
def obspy_fullresp_RESP(trace, resp_file, Address, unit='DIS', BP_filter=(0.008, 0.012, 3.0, 4.0), inform='N/N'):
    """
    Instrument correction using dataless seed ---> equivalent to full response file
    steps: detrend, demean, taper, filter, deconvolution
    """
...
def plot_tools(input, clients):
    ...
    for i in ['plot_se', 'plot_sta', 'plot_ev', 'plot_ray', 'plot_ray_gmt', 'plot_epi', 'plot_dt']:
        if input[i] != 'N':
            events, address_events = quake_info(input[i], 'info')

    ls_saved_stas = []
    ls_add_stas = []
    for k in ['plot_se', 'plot_sta', 'plot_ev', 'plot_ray', 'plot_ray_gmt', 'plot_epi']:
        if input[k] != 'N':
            for i in range(len(events)):
                ls_saved_stas_tmp = []
                ls_add_stas_tmp = []
                ...
                for j in range(len(sta_ev[0])):
                    ...
                    if input['plot_all'] != 'Y':
                        if clients == sta_ev[0][j][13]:
                            ...
                            ls_add_stas_tmp.append(os.path.join(address_events[i], BH_file,
                                                                '%s.%s.%s.%s' % (network, sta_ev[0][j][1],
                                                                                 sta_ev[0][j][2], sta_ev[0][j][3])))
...
def plot_se_ray(input, ls_saved_stas):
    ...
    for i in range(len(ls_saved_stas)):
        sys.stdout.write('\r')
        sys.stdout.write("[%-100s] %d%%" % ('='*int(100.*(i+1)/len(ls_saved_stas)), 100.*(i+1)/len(ls_saved_stas)))
        sys.stdout.flush()
        ...
        if not input['evlatmin']:
            ...
            input['evlonmin'] = -180
            input['evlonmax'] = +180
        if input['plot_se'] != 'N' or input['plot_ev'] != 'N' or input['plot_ray'] != 'N':
        ...
        if input['plot_se'] != 'N' or input['plot_ev'] != 'N' or input['plot_ray'] != 'N':
            ...
            m.scatter(x_ev, y_ev, math.log(float(ls_stas[0][12])) ** 6, color="red", marker="o", edgecolor="black",
                      zorder=10)

        for j in range(len(ls_stas)):
            try:
                station_name = '%s.%s.%s' % (ls_stas[j][1], ls_stas[j][2], ls_stas[j][3])
                ...
                if input['plot_ray'] != 'N':
                    m.drawgreatcircle(ev_lon, ev_lat, st_lon, st_lat, alpha=0.1)
                if input['plot_se'] != 'N' or input['plot_sta'] != 'N' or input['plot_ray'] != 'N':
                    x_sta, y_sta = m(st_lon, st_lat)
                    m.scatter(x_sta, y_sta, 20, color='blue', marker="o", edgecolor="black", zorder=10)
    ...
    plt.savefig(os.path.join(input['plot_save'], 'plot.%s' % input['plot_format']))
...
def plot_ray_gmt(input, ls_saved_stas):
    ...
    for i in range(len(ls_saved_stas)):
        ...
        if not input['evlatmin'] <= float(ls_stas[0][9]) <= input['evlatmax'] or \
           ...
           not input['min_mag'] <= float(ls_stas[0][12]) <= input['max_mag']:
            continue
        ev_plot_open.writelines('%s %s \n' % (round(float(ls_stas[0][10]), 5), round(float(ls_stas[0][9]), 5)))
        pattern_sta = '%s.%s.%s' % (input['sta'], input['loc'], input['cha'])
        ...
        for j in range(len(ls_stas)):
            ...
            ls_sta.append([station_ID, [str(round(float(ls_stas[j][4]), 5)), str(round(float(ls_stas[j][5]), 5))]])
    ...
    for target in range(len(ls_add_stas)):
        ...
        for i in range(len(ls_add_stas[target])):
            try:
                ...
                tr.normalize()
                dist = locations2degrees(float(ls_saved_stas[target][i][9]), float(ls_saved_stas[target][i][10]),
                                         float(ls_saved_stas[target][i][4]), float(ls_saved_stas[target][i][5]))
                if input['min_epi'] <= dist <= input['max_epi']:
                    plt.plot(np.linspace(0, (tr.stats.npts-1)/tr.stats.sampling_rate, tr.stats.npts), tr.data + dist,
                             color='black')
            except Exception as e:
                print 'WARNING: %s' % e
                pass
...
def plot_dt(input, address_events):
    ...
    for i in range(len(address_events)):
        for client_time in ['time_iris', 'time_arc']:
            print 'Event address: %s' % address_events[i]
            if os.path.isfile(os.path.join(address_events[i], 'info', client_time)):
                plt.clf()
                dt_open = open(os.path.join(address_events[i], 'info', client_time))
                ...
                for k in range(len(dt_read)):
                    ...
                    if dt_read[k][7] == '+':
                        single_succ = plt.scatter(time_single, MB_single, s=1, c='b', edgecolors='b', marker='o',
                                                  label='Serial (successful)')
                        succ += 1
                    elif dt_read[k][7] == '-':
                        single_fail = plt.scatter(time_single, MB_single, s = 1, c='r', edgecolors='r', marker='o',
                                                  label='Serial (failed)')
                        fail += 1
                ...
                if input['req_parallel'] == 'Y':
                    ...
                    time_parallel = eval(rep_par_read[4].split(',')[0]) + eval(rep_par_read[4].split(',')[1])/(1024.**2)
                    MB_parallel = eval(rep_par_read[4].split(',')[2])
                    trans_rate_parallel = MB_parallel/time_parallel*60
                    parallel_succ = plt.scatter(time_parallel, MB_parallel, s=30, c='r', edgecolors='r', marker='o',
                                                label='Parallel')
                ...
                trans_rate = (poly(time_array[-1])-poly(time_array[0]))/(time_array[-1]-time_array[0])*60
                ...
                plt_title = '%s\nAll: %s--Succ: %s (%s%%)-Fail: %s (%s%%)--%sMb/min' \
                            % (client_time.split('_')[1].upper(), (succ + fail), succ,
                               ...
                               round(float(fail)/(succ + fail)*100., 1), round(trans_rate, 2))
                plt.title(plt_title, size='x-large')

                if input['req_parallel'] == 'Y':
                    plt.legend([single_succ, parallel_succ], ['Serial', 'Parallel'], loc=4)

                plt.savefig(os.path.join(address_events[i], 'info', 'Data-Time_%s.%s' % (client_time.split('_')[1],
                                                                                         input['plot_format'])))
...
def writesac_all(i, address_events):
    ...
    for j in range(len(sta_ev[0])):
        station_id = '%s.%s.%s.%s' % (sta_ev[0][j][0], sta_ev[0][j][1], sta_ev[0][j][2], sta_ev[0][j][3])
        ls_saved_stas.append(os.path.join(address_events[i], 'BH_RAW', station_id))
    for j in range(len(sta_ev[0])):
        try:
...
def quake_info(address, target):
    ...
    for t_add in target_add:
        if not os.path.isfile(os.path.join(t_add, 'quake')):
            ...
            print 'WARNING: it uses just one seismogram to re-create the quake file'
            print '================================================================'
        ...
        quake_d = {'year0': int(quake_read_tmp[0]), 'julday0': int(quake_read_tmp[1]),
                   'hour0': int(quake_read_tmp[2]), 'minute0': int(quake_read_tmp[3]),
                   'second0': int(quake_read_tmp[4]),
                   'lat': float(quake_read_tmp[6]), 'lon': float(quake_read_tmp[7]), 'dp': float(quake_read_tmp[8]),
                   'mag': float(quake_read_tmp[9]),
                   'year1': int(quake_read_tmp[10]), 'julday1': int(quake_read_tmp[11]),
                   'hour1': int(quake_read_tmp[14]), 'minute1': int(quake_read_tmp[15]),
                   'second1': int(quake_read_tmp[16]),
                   'year2': int(quake_read_tmp[18]), 'julday2': int(quake_read_tmp[19]),
                   'hour2': int(quake_read_tmp[22]), 'minute2': int(quake_read_tmp[23]),
                   'second2': int(quake_read_tmp[24])}

        quake_t0 = UTCDateTime(year=quake_d['year0'], julday=quake_d['julday0'], hour=quake_d['hour0'],
                               minute=quake_d['minute0'], second=quake_d['second0'])
        quake_t1 = UTCDateTime(year=quake_d['year1'], julday=quake_d['julday1'], hour=quake_d['hour1'],
                               minute=quake_d['minute1'], second=quake_d['second1'])
        quake_t2 = UTCDateTime(year=quake_d['year2'], julday=quake_d['julday2'], hour=quake_d['hour2'],
                               minute=quake_d['minute2'], second=quake_d['second2'])

        events.append({'author': 'NONE', 'datetime': quake_t0, 'depth': quake_d['dp'],
                       'event_id': quake_file[5].split('-')[0].lstrip(), 'flynn_region': 'NONE',
                       'latitude': quake_d['lat'], 'longitude': quake_d['lon'], 'magnitude': quake_d['mag'],
                       'magnitude_type': 'NONE', 'origin_id': -12345.0, 't1': quake_t1, 't2': quake_t2})
...
def quake_create(address_info):
    ...
    try:
        ...
        quake_file.writelines(' '*(15 - len('%.5f' % sta_stats.sac.evla)) + '%.5f' % sta_stats.sac.evla +
                              ' '*(15 - len('%.5f' % sta_stats.sac.evlo)) + '%.5f' % sta_stats.sac.evlo + '\n')
        quake_file.writelines(' '*(15 - len('%.5f' % abs(sta_stats.sac.evdp))) + '%.5f'
                                                                                 % abs(sta_stats.sac.evdp) + '\n')
        quake_file.writelines(' '*(15 - len('%.5f' % abs(sta_stats.sac.mag))) + '%.5f' % abs(sta_stats.sac.mag) + '\n')
        quake_file.writelines(' '*(15 - len(address.split('/')[-1])) + address.split('/')[-1] + '-' + '\n')

        quake_file.writelines(repr(sta_stats.starttime.year).rjust(15) + repr(sta_stats.starttime.julday).rjust(15) +
                              repr(sta_stats.starttime.month).rjust(15) +
                              repr(sta_stats.starttime.day).rjust(15) + '\n')
        quake_file.writelines(repr(sta_stats.starttime.hour).rjust(15) + repr(sta_stats.starttime.minute).rjust(15) +
                              repr(sta_stats.starttime.second).rjust(15) +
                              repr(sta_stats.starttime.microsecond).rjust(15) + '\n')

        sta_stats_endtime = sta_stats.starttime + (sta_stats.npts-1)/sta_stats.sampling_rate

        quake_file.writelines(repr(sta_stats_endtime.year).rjust(15) + repr(sta_stats_endtime.julday).rjust(15) +
                              repr(sta_stats_endtime.month).rjust(15) + repr(sta_stats_endtime.day).rjust(15) + '\n')
        quake_file.writelines(repr(sta_stats_endtime.hour).rjust(15) + repr(sta_stats_endtime.minute).rjust(15) +
                              repr(sta_stats_endtime.second).rjust(15) +
                              repr(sta_stats_endtime.microsecond).rjust(15) + '\n')

    except Exception as e:
        print '\n================='
        print 'WARNING: Can not read all the required information from the header'
        ...
        quake_file.writelines(' '*(15 - len('%.5f' % abs(-12345.0))) + '%.5f' % abs(-12345.0) + '\n')
        quake_file.writelines(' '*(15 - len('%.5f' % abs(-12345.0))) + '%.5f' % abs(-12345.0) + '\n')
        quake_file.writelines(' '*(15 - len(address.split('/')[-1])) + address.split('/')[-1] + '-' + '\n')
...
def quake_modify(quake_item, address_info):
    ...
    try:
        quake_file_new.writelines(repr(int(quake_item[0])).rjust(15) + repr(int(quake_item[1])).rjust(15) + '\n')
        quake_file_new.writelines(repr(int(quake_item[2])).rjust(15) +
                                  repr(int(quake_item[3])).rjust(15) + repr(int(quake_item[4])).rjust(15) +
                                  repr(int(quake_item[5])).rjust(15) + '\n')
        quake_file_new.writelines(' '*(15 - len('%.5f' % quake_item[6])) + '%.5f' % quake_item[6] +
                                  ' '*(15 - len('%.5f' % quake_item[7])) + '%.5f' % quake_item[7] + '\n')
        quake_file_new.writelines(' '*(15 - len('%.5f' % abs(quake_item[8]))) + '%.5f' % abs(quake_item[8]) + '\n')
        quake_file_new.writelines(' '*(15 - len('%.5f' % abs(sta_stats.sac.mag))) + '%.5f'
                                                                                    % abs(sta_stats.sac.mag) + '\n')
        quake_file_new.writelines(' '*(15 - len(address.split('/')[-1])) + address.split('/')[-1] + '-' + '\n')
        ...
        print '=================\n'
        quake_file_new.writelines(repr(int(quake_item[0])).rjust(15) + repr(int(quake_item[1])).rjust(15) + '\n')
        quake_file_new.writelines(repr(int(quake_item[2])).rjust(15) + repr(int(quake_item[3])).rjust(15) +
                                  repr(int(quake_item[4])).rjust(15) + repr(int(quake_item[5])).rjust(15) + '\n')
        quake_file_new.writelines(' '*(15 - len('%.5f' % quake_item[6])) + '%.5f' % quake_item[6] +
                                  ' '*(15 - len('%.5f' % quake_item[7])) + '%.5f' % quake_item[7] + '\n')
        quake_file_new.writelines(' '*(15 - len('%.5f' % abs(quake_item[8]))) + '%.5f' % abs(quake_item[8]) + '\n')
        quake_file_new.writelines(' '*(15 - len('%.5f' % abs(-12345.0))) + '%.5f' % abs(-12345.0) + '\n')
        quake_file_new.writelines(' '*(15 - len(address.split('/')[-1])) + address.split('/')[-1] + '-' + '\n')
